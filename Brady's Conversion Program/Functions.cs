...
*/
                ...
                try {
                    ...
                    if (existingPatient == null) {
                        ...
                        DmgPatient newPatient = new DmgPatient {
                            ...
                            LastModifiedBy = 0,
                            AccountBalenceExists = false,
                            AddressId = 0,
                            AsssignedPhysicianId = 0,
                            HasAlerts = false,
                            LastExamAppointmentTypeId = 0,
                            ConversionDetailId = 0,
                            EmploymentStatusId = 0,
                            HasRemarks = false,
                            IsMerge = false,
                            IsTestPatient = false,
                            LastExamBillingLocationId = 0,
                            LastExamResourceId = 0,
                            MergeDetailId = 0,
                            LastExamStaffLocationId = 0,
                            OnHold = false,
                        ...
                        var patientAdditionalDetail = new DmgPatientAdditionalDetail {
                            ...
                            DefaultLocationId = 0,
                            DoNotContactAutomatedPhone = false,
                            EmergencyAddressId = 0,
                            EmployerAddressId = 0,
                            DoNotContactEmail = false,
                            DoNotContactHumanPhone = false,
                            PreferredLangaugeId = 0,
                            DoNotContactPostal = false,
                            EmergencyFirst = "",
                            EmergencyLast = "",
                            EmergencyPatientId = 0,
                            ...
                            IsStudent = false,
                            Npp = false,
                            NppDate = minAcceptableDate,
                            OrganizationId = 0,
                            PreferredDoNotContact = false
                        };
            ...
            foreach (var appointment in convAppointments) {
                ...
                try {
                    ...
                    if (!string.IsNullOrEmpty(appointment.OldBillingLocationId)) {
                        ...
                        var staffLoc = ffpmDbContext.Locations.FirstOrDefault(l => l.CompanyId.ToString() == tempLoc);
                        locationId = (int)(staffLoc?.LocationId ?? 1);
                    }
                    ...
                    if (!string.IsNullOrEmpty(appointment.OldBillingLocationId)) {
                        ...
                        if (tempLoc == "N/A") {
                            ...
                            var loc = ffpmDbContext.BillingLocations.FirstOrDefault(l => l.Code == tempLoc);
                            billingLocId = loc?.BillingLocationId ?? locationId;
                        }
                    }
                    ...
                    if (!string.IsNullOrEmpty(appointment.OldAppointmentTypeId)) {
                        typeXrefCode = appointmentTypeCodes.GetValueOrDefault(appointment.OldAppointmentTypeId) ?? "";
                    }
                    var typeXref = appointmentTypes.FirstOrDefault(s => s.Code == typeXrefCode);
        ...
        public static void ConvertLocation(List<Models.Location> convLocations, FoxfireConvContext convDbContext, FfpmContext ffpmDbContext, ILogger logger, ILogger report, ProgressBar progress,
            ...
            foreach (var location in convLocations) {
                ...
                try {
                    ...
                    bool isBilling = location.IsBilling == null || !(location.IsBilling.ToLower() == "no" || location.IsBilling == "0" || location.IsBilling.ToLower() == "f"
                        || location.IsBilling.ToLower() == "false" || location.IsBilling.ToLower() == "n");

                    bool isSchedule = location.IsScheduling == null || !(location.IsScheduling.ToLower() == "no" || location.IsScheduling == "0" || location.IsScheduling.ToLower() == "f"
                        || location.IsScheduling.ToLower() == "false" || location.IsScheduling.ToLower() == "n");
                ...
                try {
                    ...
                    if (ffpmOrig == null) {
                        var newPatientInsurance = new Brady_s_Conversion_Program.ModelsA.DmgPatientInsurance {
                            ...
                            AddedBy = 0,
                            AddedTime = minAcceptableDate,
                            AssignmentOfBenefits = false,
                            AssignmentOfBenefitsDate = minAcceptableDate,
                            InsurancePolicyType = false,
                            IsVerified = false,
                            OldRecord = "",
                            RelationId = 0,
                            RemovedBy = 0,
                            RemovedTime = minAcceptableDate,
                            SubscriberId = 0,
                            VerificationDate = minAcceptableDate
                        };
                ...
                try {
                    ...
                    if (ffpmOrig == null) {
                        var newRecallList = new Brady_s_Conversion_Program.ModelsA.SchedulingPatientRecallList {
                            ...
                            DateTimeAdded = minAcceptableDate,
        ...
        public static void ConvertProvider(List<Models.Provider> convProviders, FoxfireConvContext convDbContext, FfpmContext ffpmDbContext, ILogger logger, ILogger report, ProgressBar progress,
            ...
            foreach (var provider in convProviders) {
                ...
                try {
                    ...
                    DateTime dobDate = minAcceptableDate;
                    ...
                    string providerCode = tempCode ?? "";
                    if (string.IsNullOrEmpty(providerCode)) {
                        logger.Log($"Conv: Conv Provider code xref not found for provider with ID: {provider.Id}");
                        failed++;
                        continue;
                    } else if (providerCode == "N/A") {
                        logger.Log($"Conv: Provider code is N/A for provider with ID: {provider.Id}");
                        continue;
                    }
                    ...
                    if (isScheduling) {
                        tempCode = appointmentProviderCodes.GetValueOrDefault(provider.OldProviderCode);
                        providerCode = tempCode ?? "";
                        if (string.IsNullOrEmpty(providerCode)) {
                            logger.Log($"Conv: Xref Provider code not found for provider with ID: {provider.Id}");
                            failed++;
                            continue;
                        } else if (providerCode == "N/A") {
                            logger.Log($"Conv: Provider code is N/A for provider with ID: {provider.Id}");
            ...
            var addressesToAdd = new List<Brady_s_Conversion_Program.ModelsA.DmgOtherAddress>();

            foreach (var provider in ffpmDbContext.DmgProviders) {
            ...
            ffpmDbContext.DmgOtherAddresses.AddRange(addressesToAdd);
            ffpmDbContext.SaveChanges();

            var updatedAddresses = ffpmDbContext.DmgOtherAddresses.Where(address => address.OwnerType == 3).ToList();

            foreach (var address in updatedAddresses) {
                var provider = ffpmDbContext.DmgProviders.FirstOrDefault(p => p.ProviderId == address.OwnerId);
                if (provider != null) {
                    provider.ProviderAddressId = address.AddressId;
                }
            }
            ...
            foreach (var recallType in convRecallTypes) {
                ...
                try {
                    ...
                    if (string.IsNullOrEmpty(code)) {
                        logger.Log($"Conv: Xref Recall type code not found for recall type with ID: {recallType.Id}");
                        failed++;
                        continue;
                    } else if (code == "N/A") {
                        logger.Log($"Conv: Recall type code is N/A for recall type with ID: {recallType.Id}");
                        continue;
                    }
        ...
        public static void ConvertReferral(List<Models.Referral> convReferrals, FoxfireConvContext convDbContext, FfpmContext ffpmDbContext, ILogger logger, ILogger report, ProgressBar progress,
            ...
            foreach (var referral in convReferrals) {
                ...
                try {
                    if (string.IsNullOrEmpty(referral.OldReferralCode)) {
                        logger.Log($"Conv: Conv Provider ID is null or empty for referral with ID: {referral.Id}");
                        failed++;
                        continue;
                    }
                    ...
                    if (string.IsNullOrEmpty(referralCode)) {
                        logger.Log($"Conv: xref Provider ID not found for referral with ID: {referral.Id}");
                        failed++;
                        continue;
                    } else if (referralCode == "N/A") {
                        logger.Log($"Conv: xref Provider ID is N/A for referral with ID: {referral.Id}");
                        continue;
                    }
            ...
            foreach (var referral in ffpmDbContext.ReferringProviders) {
                if (referral.RefProviderId > 0) {
                    var newAddress = new Brady_s_Conversion_Program.ModelsA.DmgOtherAddress {
                        OwnerId = referral.RefProviderId,
                        OwnerType = 4
                    };
            ...
            foreach (var address in ffpmDbContext.DmgOtherAddresses) {
                var referral = ffpmDbContext.DmgProviders.FirstOrDefault(p => p.ProviderId == address.OwnerId);
                if (referral != null) {
                    referral.ProviderAddressId = address.AddressId;
                }
            }
        }
